        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    // 글 내용
                    var txt = data.read['content'].replace(/<br?\/?>/g, " ");    					
                    //alert(txt);
                    var sTxt = txt;
                    if (data.read['content_image'] != null) {
                        var img = data.read['content_image'].replace(/<br>/g, " ");    					
                        var sImg = '<p style="margin-bottom:10px;">'+img+'</p>';
                    } else {
                        var sImg = '';
                    }
				aHtml.push('<div class="inline_block">');
                    aHtml.push('<div class="view inline_block" style="width: 990px;">');
                    aHtml.push(sImg);
                    aHtml.push('<p>'+sTxt+'</p>');
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<div class="inline_block" style="vertical-align: top;"><a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=4&link_product_no='+iProductNo+'" class="dj-btn-2-review-edit">게시글 수정</a></div>');
                    }
				aHtml.push('</div>');
                    // 댓글리스트
                ...
                //$(pNode).after('<tr id="product-review-read'+data.key+'"></tr>');
