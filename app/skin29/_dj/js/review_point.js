            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        ...
                        for (var i= (data.comment.length-1); data.comment.length >= i; i++) {
                            //댓글리스트
                ...
                if (data.comment_write != undefined && data.comment_write['use_comment_size'] == '') PRODUCT_COMMENT.comment_byte(2, data.key);
                if (data.comment_reply != undefined && data.comment_write['use_comment_size'] == '') PRODUCT_COMMENT.comment_byte(2, data.key, 'commentReplyWriteForm');
            }
        });
    },
